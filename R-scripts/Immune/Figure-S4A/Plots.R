...
# add spercategories
my.data.redo3<-FetchData(myData_LI,c("orig.ident","sampleID","Index","nCount_RNA","nFeature_RNA","percent.mt","seurat_clusters","type","TYPE","TYPES","age","pos","celltypes_old","celltypes_new","celltypes_new_redo","celltypes_new_redo2","RNA_snn_res.0.01"))
pca<-Embeddings(myData_LI, reduction = "pca")[, 1:5]
umap<-Embeddings(myData_LI, reduction = "umap")[, 1:2]
my.data.redo3<-merge(my.data.redo3,umap,by="row.names",all.x=T)
rownames(my.data.redo3)<-my.data.redo3$Row.names
my.data.redo3<-my.data.redo3[,-1]
my.data.redo3<-merge(my.data.redo3,pca,by="row.names",all.x=T)
rownames(my.data.redo3)<-my.data.redo3$Row.names
my.data.redo3<-my.data.redo3[,-1]
my.data.redo3[my.data.redo3$RNA_snn_res.0.01==0,"RNA_snn_res.0.01"]
my.data.redo3$celltypes_new_redo3<-"T_lymphocyte"
my.data.redo3[my.data.redo3$RNA_snn_res.0.01==1,"celltypes_new_redo3"]<-"B_lymphocyte"
my.data.redo3[my.data.redo3$RNA_snn_res.0.01==2,"celltypes_new_redo3"]<-"Myeloid_lineage"
write.csv(my.data.redo3,"./data_out//my.data.redo3.csv")
facet_3<-function(my.data){
my.data3<-my.data[,c("celltypes_new_redo3","age","Index","pos")]
my.data3$var<-paste(my.data3$celltypes_new_redo3,my.data3$age,my.data3$Index,my.data3$pos,sep="#")
my.data3<-as.data.frame(table(my.data3$var))
my.data3$celltypes_new_redo3<-str_split_fixed(my.data3$Var1,"#",4)[,1]
...
mycolors <- colorRampPalette(brewer.pal(12,"Paired"))(length(unique(my.data$celltypes_new_redo3)))
...
p<-ggplot(tg3, aes(fill=celltypes_new_redo3, y=ratio, x=pos)) +
...
xx2<-facet_3(my.data.redo3)
xx2$figure
yy2<-xx2$variable
yy2$celltypes_new_redo3<-factor(yy2$celltypes_new_redo3,levels = c("B_lymphocyte","T_lymphocyte","Myeloid_lineage"))
yy2$age<-factor(yy2$age,levels = c("young","old"))


### Figure S6A: Single_Immune_FacetPos_redo3.pdf 
