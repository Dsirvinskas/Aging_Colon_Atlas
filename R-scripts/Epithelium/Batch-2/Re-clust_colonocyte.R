...
## For Colonocyte cells
Colonocyte = subset(SI.integrated, idents = c("Colonocyte_1","Colonocyte_2"))
## K-means
    dat = Colonocyte@assays$integrated@data
    texpdf<-t(dat)
    tot<-data.frame(center=NULL,tot=NULL)
   for (c in 3:6) {
      k<-kmeans(texpdf,center=c,nstart=5)
      write.csv(k$cluster,paste(c,"mean_Colonocyte.csv",sep=""))
      print(c)
      
      tenmeans = read.csv(file = paste(c,"mean_Colonocyte.csv", sep = ""))
      tenmeans[,2] = as.factor(tenmeans[,2])
      row.names(tenmeans) = tenmeans[,1]
      tenmeans = tenmeans[row.names(tenmeans) %in% row.names(Colonocyte@meta.data),]
      
      Idents(Colonocyte) = tenmeans[,2]
      cols = c("Red", "Blue", "grey30", "Green", "Yellow", "Magenta", "Cyan", "indianred", "Pink", "Purple", "orangered1", "wheat4", "darkolivegreen2", "slategray3", "lightsalmon2")
      pdf(file = paste("Colonocyte_rPCA_TSNE_", c,"means.pdf", sep = "")) 
      plot = DimPlot(Colonocyte, reduction = "tsne", cols = cols) 
      print(plot)
      plot = DimPlot(Colonocyte, reduction = "tsne", cols = cols, group.by = "Compartment") 
      print(plot)
      plot = FeaturePlot(Colonocyte, reduction = "tsne", features = c("Gsdmc4", "Selenbp1", "Lgals3", "Mgat4c"), pt.size = 0.01, )
      print(plot)
      dev.off() 
      ...
      write.csv(top10, file= paste("Colonocyte", c,"meanTop10_genes_rPCA.csv", sep ="") )
      
      col.plot = c("Red", "Black", "Blue") 
      pdf(file = paste("Colonocyte_Heatmap_",c,"means_rPCA.pdf", sep = "") )
      ...
      Info = cbind(Colonocyte@meta.data, "Cluster" = Colonocyte@active.ident)
      Info$Age[which(Info$Age == "Young_20w")] = "Young"
      A = c(which(colnames(Info)=="Compartment"), which(colnames(Info)=="Age"), which(colnames(Info)=="Cluster"))
      
      Info1 = table(Info[,A])
      write.csv(Info1, file = paste(c,"mean_Colonocyte_Compartment_Numbers.csv", sep=""))
      
    pdf(file= paste(c,"mean_Colonocyte_Violin.pdf"))
    a = VlnPlot(Colonocyte, features = c("Lgr5", "Sox4", "Slc12a2","Ascl2","Mki67", "Pcna"), pt.size = 0.01)
    print(a)
    a = VlnPlot(Colonocyte, features = c("Lgr5", "Sox4", "Slc12a2","Ascl2","Mki67", "Pcna"), pt.size = 0)
    print(a)
    
    a = VlnPlot(Colonocyte, features = c("Dll1", "Atoh1","Chga", "Chgb"), pt.size = 0.01)
    print(a)
    a = VlnPlot(Colonocyte, features = c("Dll1", "Atoh1","Chga", "Chgb"), pt.size = 0)
    print(a)
    
    a = VlnPlot(Colonocyte, features = c("Reg4", "Muc2", "Agr2", "Spink1","Cd24a", "Dclk1"), pt.size = 0.01)
    print(a)
    a = VlnPlot(Colonocyte, features = c("Reg4", "Muc2", "Agr2", "Spink1","Cd24a", "Dclk1"), pt.size = 0)
    print(a)
    
    a = VlnPlot(Colonocyte, features = c("Ptprc", "H2-aa", "H2-ab1"), pt.size = 0.01)
    print(a)
    a = VlnPlot(Colonocyte, features = c("Ptprc", "H2-aa", "H2-ab1"), pt.size = 0)
    print(a)
